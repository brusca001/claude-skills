#!/usr/bin/env python3
"""
Direct Telegram send, self-contained (doesn't depend on the local `telegram`
skill being present — needed for the /schedule cloud routine, which won't
have ~/.claude/skills/telegram available). For local/manual runs, using the
existing `/telegram` skill directly works just as well.

Every message MUST start with "@MrJoben_bot" per the existing telegram skill's convention.
"""
